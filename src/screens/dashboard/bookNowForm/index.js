...
function BookNowForm(props) {
  ...
  const getOtherSymptoms = () => {
    if (symptomInput.trim() !== '') {
      const newSymptoms = symptomInput.split(',').map((symptom) => symptom.trim());
      // Filter out duplicates and return the new symptoms
      return [...new Set(newSymptoms)];
    }
    return [];
  };
  const BookNowPress = () => {
    // Ensure symptomInput is not empty
    if (isButtonEnabled && symptomInput.trim() !== '') {
      const concatenatedArray = [...selectedSymptoms, ...getOtherSymptoms()]; // Combine selected symptoms and other symptoms
  ...
  useEffect(() => {
    checkFormCompletion();
  }, [selectedOption, selectedDate, selectedBookingSlot, selectedSymptoms, symptomsList, symptomInput]);
  const checkFormCompletion = () => {
    if (
      (selectedOption === 'VISIT' || selectedOption === 'VACCINATION') &&
      selectedBookingSlot !== null &&
      // selectedDate !== null &&
      selectedSymptoms.length > 0 || symptomsList.length > 0 || symptomInput.trim() !== ''
  
    ) {
      setIsButtonEnabled(true); // Enable the button for 'VISIT' or 'VACCINATION' with a selected slot or if other symptoms or symptomInput is provided
